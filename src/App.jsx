...
import { LayoutDashboard, CalendarDays, GitBranch, BookHeart, Users, Settings, LogOut, LogIn, Bell, Moon, SunMedium, Menu, X, Sun, ChevronRight, Plus, Search, MapPin, Trash2, List, Calendar, ArrowUpRight, Check, Leaf, Heart, Mail, RefreshCw, Smartphone, Clock3, Sparkles, Contact, Cake, Phone, PauseCircle, PlayCircle, Network, ListTree } from 'lucide-react';
import { TongQuan, LichNgayGio, GiaPha, NguoiConSong, MienKyUc, ThanhVien, TaiKhoan, LoiNhac, TimKiem, MenuIcon, MuiTenPhai } from './icons.jsx';
...
import { dangBat, doiChuyenDong, useLoDan } from './motion.js';
...
export default function App(){
  ...
  // Trang của một người mở được từ trang chủ, từ lịch, từ gia phả hay từ miền ký ức.
  // Nhớ lại nơi vừa rời đi thì nút lùi trả về đúng chỗ ấy, thay vì luôn đổ về gia phả.
  function openDetail(person){setActionError('');setModal(null);if(route.section!=='person')setTuDau(route.section);go('person/'+person.id);}
  function lui(){go(route.section==='person'?tuDau:'overview');}
  // Địa chỉ ảnh đại diện kèm mã ảnh, để đổi ảnh xong trình duyệt không giữ lại ảnh cũ.
  const anhTaiKhoan=u=>u?.avatar_id||u?.has_avatar?`/api/members/${u.id}/avatar?v=${u.avatar_id||'1'}`:null;
  /** Tải ảnh thẳng từ sơ đồ gia phả, không phải mở trang riêng của từng người. */
  async function themAnhNhanh(person,file){
    setActionError('');setDangTaiAnh(person.id);
    try{
      const r=await api(`/ancestors/${person.id}/photos`,{method:'POST',body:{data:await preparePortrait(file),caption:''}});
      await reload();
      setToast(r.status==='pending'
        ?`Đã gửi ảnh của ${person.name}. Người quản lý duyệt xong thì cả họ sẽ thấy.`
        :r.portrait?`Đã đặt ảnh cho ${person.name}.`:`Đã thêm ảnh vào album của ${person.name}.`);
    }catch(e){setToast(e.message);}
    finally{setDangTaiAnh(null);}
  }
  ...
  // Trang của một người và trang tài khoản không nằm trong thanh điều hướng, nên chúng
  // là nơi duy nhất cần một đường lùi; ở các mục chính thì chỗ đó là nút mở menu.
  const trangCon=section==='person'||section==='account';
  ...
  function personCard(p){return <button className={`ancestor-node mo-lo ${p.living?'living':''}`} onClick={()=>openDetail(personEvent(p))}><Avatar name={p.name} photoId={p.photo_id}/><strong>{p.name}</strong><span>{p.living?(p.birth_year?`Sinh ${p.birth_year}`:'Còn sống'):`${p.birth_year||'…'} — ${p.death_year||'…'}`}</span><small>{p.living?`Đời thứ ${p.generation}`:`Giỗ ${pad(p.lunar_day)}/${pad(p.lunar_month)} âm`}</small></button>;}
  ...
  return <div className="app-shell"><KhongKhi/><a className="skip-link" href="#main-content" onClick={e=>{e.preventDefault();document.getElementById('main-content').focus();}}>Đến nội dung chính</a>{mobileMenu&&<button className="nav-overlay" aria-label="Đóng menu" onClick={()=>setMobileMenu(false)}/>}<aside className={`sidebar ${mobileMenu?'open':''}`}><div className="sidebar-top"><button className="brand-link" onClick={()=>go('overview')} aria-label="Về tổng quan Đỗ Gia"><Brand/></button><span className="brand-tagline">NƠI GỐC RỄ CÒN MÃI</span></div><div className="clan-badge"><span className="clan-initial">{data.family.name.replace('Dòng họ ','')[0]}</span><div><strong>{data.family.name}</strong><small>{data.members.length} thành viên</small></div></div><div className="nav-section-label">KHÔNG GIAN GIA ĐÌNH</div><nav aria-label="Điều hướng chính">{navigation.filter(([id])=>!guest||(id!=='members'&&id!=='living')).map(([id,label,Icon])=><button key={id} className={`nav-link ${section===id?'active':''}`} onClick={()=>go(id)} aria-current={section===id?'page':undefined}><Icon/><span>{label}</span>{id==='calendar'&&<small>{events.filter(e=>e.daysAway<30).length}</small>}{id==='memories'&&admin&&pendingMemories.length>0&&<small className="badge-pending">{pendingMemories.length}</small>}{section===id&&<MuiTenPhai className="nav-caret"/>}</button>)}</nav><div className="sidebar-bottom"><div className="sidebar-quote"><Leaf/><p>“Cây có cội,<br/>nước có nguồn.”</p></div>{guest?<button className="nav-link highlight" onClick={()=>setShowLogin(true)}><LogIn/>Đăng nhập · Đăng ký</button>:<><button className={`nav-link ${section==='account'?'active':''}`} onClick={()=>openSettings('profile')}><TaiKhoan/>Tài khoản & lời nhắc</button><button className="sidebar-profile" onClick={()=>openSettings('profile')}><Avatar name={data.user.name} src={anhTaiKhoan(data.user)}/><span><strong>{data.user.name}</strong><small>{admin?'Người quản lý':'Thành viên gia đình'}</small></span><ChevronRight/></button><button className="logout" onClick={logout}><LogOut/>Đăng xuất</button></>}</div></aside><nav className="mobile-tabs" aria-label="Điều hướng nhanh">{mobileTabs.map(([id,label,Icon])=><button key={id} className={`tab ${section===id?'active':''}`} onClick={()=>go(id)} aria-current={section===id?'page':undefined}><span className="tab-ico"><Icon/></span><span>{label}</span></button>)}<button className={`tab ${section==='account'?'active':''}`} onClick={()=>guest?setShowLogin(true):openSettings('profile')} aria-current={section==='account'?'page':undefined}><span className="tab-ico"><TaiKhoan/></span><span>{guest?'Đăng nhập':'Tài khoản'}</span></button></nav><div className="workspace"><header className="topbar"><div className="breadcrumb">{trangCon
        ? <button className="icon-button back-button" onClick={lui} aria-label={`Quay lại ${navigation.find(x=>x[0]===tuDau)?.[1]||'Tổng quan'}`}><MuiTenPhai/></button>
        : <button className="icon-button mobile-toggle" onClick={()=>setMobileMenu(true)} aria-label="Mở menu"><MenuIcon/></button>}<span>{trangCon?(navigation.find(x=>x[0]===tuDau)?.[1]||'Tổng quan'):'Gia đình'}</span><ChevronRight/><strong>{activeLabel}</strong></div><div className="topbar-right"><span className="today-label"><Sun/>{solarLabel(today,{weekday:'long',day:'numeric',month:'long'})}</span><button className="icon-button theme-toggle" onClick={doiGiaoDien} aria-label={`Giao diện ${TEN[giaoDien].toLowerCase()}. Bấm để đổi.`} title={giaoDien==='auto'?`Theo giờ Việt Nam — đang ${dangDung==='light'?'sáng':'tối'}`:`Giao diện ${TEN[giaoDien].toLowerCase()}`}>{giaoDien==='auto'?<Clock3/>:giaoDien==='light'?<SunMedium/>:<Moon/>}</button><button className="icon-button motion-toggle" onClick={()=>setChuyenDong(doiChuyenDong())} aria-pressed={!chuyenDong} aria-label={chuyenDong?'Tạm dừng chuyển động':'Bật lại chuyển động'} title={chuyenDong?'Tạm dừng chuyển động':'Bật lại chuyển động'}>{chuyenDong?<PauseCircle/>:<PlayCircle/>}</button>{guest&&<Button variant="primary" className="topbar-login" onClick={()=>setShowLogin(true)}><LogIn/>Đăng nhập</Button>}{!guest&&<><button className="icon-button" aria-label="Cài đặt thông báo" onClick={()=>openSettings()}><LoiNhac/></button><button className="profile-mini" aria-label="Tài khoản của tôi" onClick={()=>openSettings('profile')}><Avatar name={data.user.name} src={anhTaiKhoan(data.user)}/></button></>}</div></header>{data.demo&&<div className="demo-banner"><span><Leaf/>Bạn đang khám phá Đỗ Gia bằng dữ liệu minh họa.</span><button onClick={logout}>Đăng nhập dòng họ của tôi<ArrowUpRight/></button></div>}<main id="main-content" tabIndex={-1} className="main-content"><div className="page-view" key={section==='person'?'person/'+route.param:section}>
      {section==='overview'&&<><div className="overview-heading"><div>{/* Logo đứng trên câu mở đầu: trên điện thoại sidebar bị ẩn nên
        đây là chỗ duy nhất người ta còn nhìn thấy tên ứng dụng, mà lại không phải tranh
        chỗ với tên mục trên thanh đầu. */}
        <div className="overview-brand"><Brand/></div><p className="eyebrow">{guest?'Nơi dòng họ gìn giữ những ngày cần nhớ':`Chào ${data.user.name.split(' ').at(-1)}, mừng bạn trở về`}</p><h1>Gìn giữ nếp nhà.<br/><em>Kết nối thế hệ.</em></h1><p className="intro-caption">Một lời nhắc nhỏ, trọn vẹn lòng thành.</p></div><div className="overview-aside"><span className="pill"><span className="status-dot"/>Tháng {lunarDate(today).month} âm · {lunarYearName(today)}</span>{admin&&<Button onClick={openAdd}><Plus/>Thêm ngày giỗ</Button>}</div></div>{upcoming?<EventHero event={upcoming} today={today} onOpen={openDetail} onRemind={guest?undefined:openReminder} reminded={remindActive(upcoming.id)}/>:empty}<div className="overview-lower"><section className="upcoming-panel mo-dan"><div className="section-bar"><h2>Những ngày cần nhớ</h2><Button variant="text" onClick={()=>go('calendar')}>Xem lịch<ArrowUpRight/></Button></div>{events.slice(1,4).map(event=><EventRow key={event.key} event={event} today={today} onOpen={openDetail} onRemind={guest?undefined:openReminder} reminded={remindActive(event.id)}/>)}{events.length<=1&&<p className="muted">Những ngày giỗ tiếp theo sẽ được hiển thị tại đây.</p>}{upcomingRites.length>0&&<><div className="section-bar tight"><h3><Sparkles/>Việc họ sắp tới</h3></div>{upcomingRites.slice(0,2).map(rite=><ObservanceRow key={rite.key} event={rite} today={today}/>)}</>}{guest?<div className="guest-cta"><div><strong>Bạn là người trong họ?</strong><span>Đăng nhập để xem sơ đồ gia phả đầy đủ, số điện thoại liên lạc và sinh nhật của người trong họ.</span></div><Button variant="primary" onClick={()=>setShowLogin(true)}><LogIn/>Đăng nhập · Đăng ký</Button></div>:<div className="family-strip"><div className="avatar-stack">{data.members.slice(0,3).map(m=><Avatar key={m.id} name={m.name} src={anhTaiKhoan(m)}/>)}</div><div><strong>Cùng nhau giữ một nếp nhà.</strong><span>{data.members.length} thành viên · {data.ancestors.length} người trong gia phả · {departed.length} người được tưởng nhớ</span></div><Heart/></div>}</section><CalendarView {...month} events={monthEvents} observances={monthRites} birthdays={monthBirthdays} today={today} selected={selected} onSelect={date=>{setSelected(date);go('calendar');}} onMonth={setMonth} compact/></div></>}
      ...
      {section==='family'&&<><PageHeading eyebrow="Từ một gốc, nhiều thế hệ" title="Sơ đồ gia phả" description={`${data.ancestors.length} người · ${generations} đời · ${departed.length} người đã khuất.`} action={admin&&<AddButton onClick={openAdd} label="Thêm người"/>}/>{peopleFilters}{!data.ancestors.length?empty:search||branch!=='all'?<div className="ancestor-card-grid">{filteredAll.map(p=><div key={p.id}>{personCard(p)}</div>)}{!filteredAll.length&&<p className="muted">Không tìm thấy người thân phù hợp.</p>}</div>:<><div className="tree-view-tabs" role="group" aria-label="Cách xem gia phả"><button className={kieuCay==='so-do'?'active':''} aria-pressed={kieuCay==='so-do'} onClick={()=>doiKieuCay('so-do')}><Network/>Sơ đồ</button><button className={kieuCay==='nhanh'?'active':''} aria-pressed={kieuCay==='nhanh'} onClick={()=>doiKieuCay('nhanh')}><ListTree/>Theo nhánh</button></div><div className="tree-legend"><span><i className="departed"/>Đã khuất</span><span><i className="living"/>Còn sống</span><span className="muted">{kieuCay==='so-do'?'Kéo để đi, chạm một tên để xem đường về gốc':'Chạm tên để xem trang riêng'}</span></div>{kieuCay==='so-do'
        ?<TreeCanvas key="so-do" people={data.ancestors} onOpen={p=>openDetail(personEvent(p))} onPhoto={guest?null:themAnhNhanh} dangTai={dangTaiAnh}/>
        :<FamilyTree key="nhanh" people={data.ancestors} onOpen={p=>openDetail(personEvent(p))} onPhoto={guest?null:themAnhNhanh} dangTai={dangTaiAnh}/>}</>}<p className="calendar-footnote">{guest?'Khách xem được phần tưởng nhớ. Người còn sống trong họ chỉ hiện ra sau khi đăng nhập.':'Anh chị em xếp theo thứ tự sinh trong nhà. Ai chưa có thứ tự thì xếp xuống cuối.'}</p></>}
      ...
      {section==='living'&&!guest&&<><PageHeading eyebrow="Gọi được cho nhau khi cần" title="Người còn sống" description="Danh bạ trong họ. Chỉ thành viên đã đăng nhập mới xem được." action={admin&&<AddButton onClick={openAdd}/>}/>{peopleFilters}
        {!filteredAlive.length?<Empty title="Chưa có ai trong danh bạ." description={admin?'Thêm người thân và bật “Người này còn sống” để họ hiện ở đây.':'Người quản lý sẽ cập nhật danh bạ của dòng họ tại đây.'} action={admin&&<AddButton onClick={openAdd}/>}/>
          :<div className="living-grid">{filteredAlive.map(p=>{
            ...
            return <article className="living-card mo-lo" key={p.id}>
      ...
      </div><footer className="app-footer"><span>{data.family.name}</span><span className="footer-author">Đỗ Duy Phong thực hiện</span><span>Lưu giữ hôm qua. Gắn kết hôm nay.</span></footer>
...
/** Lớp không khí: một quầng sáng chậm và vài hạt bụi vàng trôi lên, nằm dưới toàn
 * bộ nội dung và không nhận sự kiện con trỏ. Cả ứng dụng dùng chung đúng một lớp
 * này, thay vì mỗi cái thẻ tự phát sáng một kiểu — đó là chỗ hiệu ứng nền hay
 * hỏng nhất, khi mười thứ cùng nhấp nháy thì chẳng còn thứ gì đáng nhìn.
 *
 * Vị trí các hạt tính bằng công thức chứ không bốc ngẫu nhiên: tải lại trang mà
 * chúng nhảy sang chỗ khác thì thành ra một trang khác, không phải trang cũ. */
function KhongKhi(){
  return <div className="mo-khong-khi" aria-hidden="true">
    <span className="mo-quang mot"/><span className="mo-quang hai"/>
    {Array.from({length:10},(_,i)=><i key={i} style={{
      left:`${6+(i*37)%88}%`, top:`${18+(i*29)%74}%`,
      '--mo-lau':`${11+(i%5)*2.4}s`, '--mo-tre':`${-(i*1.7).toFixed(1)}s`,
    }}/>)}
  </div>;
}
