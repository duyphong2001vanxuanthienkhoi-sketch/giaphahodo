...
import { LayoutDashboard, CalendarDays, GitBranch, BookHeart, Users, Settings, LogOut, Bell, Menu, X, Sun, ChevronRight, Plus, Search, MapPin, Trash2, List, Calendar, ArrowUpRight, Check, Leaf, Heart, Mail, RefreshCw, Smartphone, Clock3, Sparkles, Contact, Cake, Phone } from 'lucide-react';
...
const navigation=[['overview','Tổng quan',LayoutDashboard],['calendar','Lịch ngày giỗ',CalendarDays],['family','Gia phả',GitBranch],['living','Người còn sống',Contact],['memories','Miền ký ức',BookHeart],['members','Thành viên',Users]];
...
export default function App(){
  ...
  if(loading)return <div className="app-loading"><Brand/><span>Đang mở nếp nhà…</span></div>;
  ...
  function personCard(p){return <button className="ancestor-node" onClick={()=>openDetail(personEvent(p))}><Avatar name={p.name} photoId={p.photo_id}/><strong>{p.name}</strong><span>{p.birth_year||'…'} — {p.death_year||'…'}</span><small>Giỗ {pad(p.lunar_day)}/{pad(p.lunar_month)} âm</small></button>;}
  function treeNode(p,depth=0){const children=departed.filter(c=>c.parent_id===p.id);return <li key={p.id}><div className="node-generation">ĐỜI THỨ {p.generation}</div>{personCard(p)}{children.length>0&&depth<30&&<ul>{children.map(c=>treeNode(c,depth+1))}</ul>}</li>;}
  const peopleFilters=<div className="people-filters"><label className="search-field"><Search/><input aria-label="Tìm người thân" placeholder="Tìm người thân…" value={search} onChange={e=>setSearch(e.target.value)}/></label><select aria-label="Lọc theo chi" value={branch} onChange={e=>setBranch(e.target.value)}><option value="all">Tất cả chi</option>{[...new Set(data.ancestors.map(a=>a.branch))].map(b=><option key={b}>{b}</option>)}</select></div>;
        ...
        {!filteredAlive.length?<Empty title="Chưa có ai trong danh bạ." description={admin?'Thêm người thân và bật “Người này còn sống” để họ hiện ở đây.':'Người quản lý sẽ cập nhật danh bạ của dòng họ tại đây.'} action={admin&&<AddButton onClick={openAdd}/>}/>
          :<div className="living-grid">{filteredAlive.map(p=>{
            ...
            return <article className="living-card" key={p.id}>
              <button className="living-head" onClick={()=>openDetail(p)}><Avatar name={p.name} photoId={p.photo_id}/><div><strong>{p.name}</strong><span>Đời thứ {p.generation} · {p.branch}</span></div></button>
              {p.birth_date&&<p className="living-meta"><Cake/>{solarLabel(p.birth_date)}{bd?` · còn ${bd.daysAway} ngày nữa tròn ${bd.turning} tuổi`:''}</p>}
              {p.phone
                ? <a className="living-phone" href={`tel:${p.phone.replace(/\s/g,'')}`}><Phone/>{p.phone}</a>
